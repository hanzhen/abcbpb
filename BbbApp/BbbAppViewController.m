...
/*
 ...
 so far:  loaded correct data for english only a-b-c letter game.  have 70 strings, 26 different letters.
 
 
 need to modify content-view-controller, and build the proper strings below.
 
 will need serno to "go to specific" letter...?? 
...
#import "Constants.h"
#define TILES_PERPAGE  3  // put in implementation file. also in header file. 3 for ABC letter game
...
//    NSLog(@"updateCurrentIndex, nmbr currentIndex is %d", _currentIndex);     
}
...
{
//        NSLog(@"in turnToPage, with currentIndex of %d and pageID of %d", _currentIndex, pageNBR);
    
    pageNBR = pageNBR - 1;
//    NSLog(@"pageNBR now %d<", pageNBR);
    
    _currentIndex = pageNBR;
...
@synthesize L1String_Setting, L2String_Setting, LPhoneString_Setting;
...
//    NSLog(@"databaseName is...%@", databaseName );
    
    // READ IN ALL THE Version info from the database ========START  ===============100916=========>>>>>>>>>>>>>>>>>
    
	if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
		// Setup the SQL Statement and compile it for faster access
		const char *sqlStatementVc = "select abclgVNMB, abclgVModifiedDT, abclgVComment from abclgV "; // identify active sets
...
//                NSLog(@"\tWrote out databaseVersion as: %@", dbVersionString);
            ...
            NSLog(@"Error with preparing select statement to get version.");
...
//    }
    ...
    if(sqlite3_open([databasePath UTF8String], &database) == SQLITE_OK) {
        // Setup the SQL Statement and compile it for faster access
        const char *sqlStatement3 = "select serno, tileSetIDX, setID, letterTXT, tileWordPicImageNM, tileTXT_EN, tileTXT_CN, tileTXT_DE, tileTXT_ES, tileTXT_FR, tileTXT_IT, tileTXT_JP, tileTXT_KR, tileNM_cn, tileNM_jp, tileNM_kr from aTilesV1 order by setID, letterNO; ";

//        const char *sqlStatement3 = "select serno, tileSetIDX, setID, letterTXT, tileWordPicImageNM, tileTXT_EN, tileTXT_CN, tileTXT_DE from aTilesV1 order by setID, letterNO; ";
        
        
//        tileNM_CN, tileTXT_CN, tileTXT_DE, tileColor_DE, tileTXT_EN, tileTXT_ES, tileColor_ES, tileTXT_FR, tileColor_FR, tileTXT_IT, tileColor_IT, tileNM_JP, tileTXT_JP, tileNM_KR, tileTXT_KR from  WTilesV4 where tileStatus = 1 order by setID, tileID;"; // identify active tiles by set in order
...
//            NSLog(@"Now getting set %d...", mm);
            ...
            while(sqlite3_step(compiledStatement2) == SQLITE_ROW) {
                ...
                tileID_db[mm]            = sqlite3_column_int(compiledStatement2, 0);  // seqn number
                tileSetIDX[mm]           = sqlite3_column_int(compiledStatement2, 1);  // seqn number
                tileSetNM[mm]            = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,2)];
                tileLetterTXT[mm]        = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,3)];
                tilePicPicImageNM_db[mm] = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,4)];
                tileTXT_en[mm]           = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,5)];
                
                 tileTXT_cn[mm]           = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,6)];
                 tileTXT_de[mm]           = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,7)];
                 tileTXT_es[mm]           = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,8)];
                
                 tileTXT_fr[mm]           = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,9)];
                 tileTXT_it[mm]           = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,10)];
                 tileTXT_jp[mm]           = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,11)];
                 tileTXT_kr[mm]           = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,12)];

                tileNM_cn[mm]  = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,13)];
                tileNM_jp [mm]  = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,14)];
                tileNM_kr [mm]  = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,15)];
                
//                tileID_db[mm]    = sqlite3_column_int(compiledStatement2, 0);  // seqn number
//                tileSetID_db[mm] = sqlite3_column_int(compiledStatement2, 1);  // uselss count of 1 to 3... used in order by only, so remove from select
                
//                tileSetNM[mm] = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,1)];
//                tileLetterTXT[mm] = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,2)];
                
//                tilePicPicImageNM_db[mm] = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,3)];
...
//                tileTXT_en[mm] = [[NSString alloc] initWithUTF8String: (char*) sqlite3_column_text(compiledStatement2,4)];
...
/*
			...
			NSLog(@"We got for tile %d ==>\ntileID_db: %d \ttileSetIDX: %d\nTileSetNM: %@\ntileLetterTXT: %@\ntilePicImageNM_db: %@\nTileTXT_en: %@\n\n", mm, tileID_db[mm], tileSetIDX[mm],
                  tileSetNM[mm], tileLetterTXT[mm], tilePicPicImageNM_db[mm], tileTXT_en[mm]);
                
  */
                
                // halprog  above debugs getting tile data
...
//            NSLog(@"finished with jj - found %d tiles.", tileCNT); //
            ...
            NSLog(@"Error with preparing select statement to get tiles.");
...
//                NSLog(@"We got for idx %d =======>\nlangID[nn]: %d \tlangNM_EN[nn]: %@\n\n", nn, langID[nn], langNM_EN[nn]); // debug
...
// NOTE WELL: lang1 will ALWAYS be 2, or kEN for this ABCpb app.
...
- (void) createSettingsLables {    // working here -- need to do this each time kLPhone is changed... or each execution... 
//    NSLog(@"in create settings labels...  current lang is %d", kL1);
    // need to populate settingsLabelDelimitedStringHolder
    
    if (kLPhone == kCN) {
        settingsLabelDelimitedStringHolder =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%@:", kLPhone, kL2, langNM_CN[0]];
        for (int r = 1; r<9; r++) {
                      [settingsLabelDelimitedStringHolder  appendFormat:@"%@:", langNM_CN[r]];
        }
    } else if (kLPhone == kDE) {
        settingsLabelDelimitedStringHolder =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%@:", kLPhone, kL2, langNM_DE[0]];
        for (int r = 1; r<9; r++) {
            [settingsLabelDelimitedStringHolder  appendFormat:@"%@:", langNM_DE[r]];
        }
    } else if (kLPhone == kEN) {
        settingsLabelDelimitedStringHolder =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%@:", kLPhone, kL2, langNM_EN[0]];
        for (int r = 1; r<9; r++) {
            [settingsLabelDelimitedStringHolder  appendFormat:@"%@:", langNM_EN[r]];
        }
    } else if (kLPhone == kES) {
        settingsLabelDelimitedStringHolder =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%@:", kLPhone, kL2, langNM_ES[0]];
        for (int r = 1; r<9; r++) {
            [settingsLabelDelimitedStringHolder  appendFormat:@"%@:", langNM_ES[r]];
        }
    } else if (kLPhone == kFR) {
        settingsLabelDelimitedStringHolder =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%@:", kLPhone, kL2, langNM_FR[0]];
        for (int r = 1; r<9; r++) {
            [settingsLabelDelimitedStringHolder  appendFormat:@"%@:", langNM_FR[r]];
        }
    } else if (kLPhone == kIT) {
        settingsLabelDelimitedStringHolder =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%@:", kLPhone, kL2, langNM_IT[0]];
        for (int r = 1; r<9; r++) {
            [settingsLabelDelimitedStringHolder  appendFormat:@"%@:", langNM_IT[r]];
        }
    } else if (kLPhone == kJP) {
        settingsLabelDelimitedStringHolder =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%@:", kLPhone, kL2, langNM_JP[0]];
        for (int r = 1; r<9; r++) {
            [settingsLabelDelimitedStringHolder  appendFormat:@"%@:", langNM_JP[r]];
        }
    } else if (kLPhone == kKR) {
        settingsLabelDelimitedStringHolder =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%@:", kLPhone, kL2, langNM_KR[0]];
        for (int r = 1; r<9; r++) {
...
//    NSMutableString *workingPageString[TILESTBLSIZE + 1];  // add lables as finale string.
    ...
    NSMutableString *workingPageString[TILESTBLSIZE];  // for A-b-C just one row per letter... 
...
//    currentContentPageString = [[NSMutableString alloc] init]; // hold pageString content as being built
    ...
    txtWordL2  = [[NSMutableString alloc] init]; // hold L2 word text for each word on the page; used four times per page. not used yet.
    
    currentLetterNM = [[NSMutableString alloc] initWithString: @"Aa"]; // hold set name for creation of one page.  Aa, Bb, etc.
...
//    tileNO = tileID_db[0]; // serno
    tileNO = 0;
    
    currentPageNO = 1; currentTileNbrOnPage = 1;

    currentSetNO = 1; // is first
    
//    NSLog(@"process first tile...  %d, current tileNbrOnPage is %d<", tileNO, currentTileNbrOnPage);  // trackhal
    
    //  get current tile number.
//    currentSetNO = tileSetIDX[0];  // first tile's set. // bot used in ABC

    
    /// halprog note:  hardcoded 4 as number on page, and tileno's 1/2/3/4 -- not using these values for now 121027 ver 1.04
    
    
    workingPageString[currentPageNO] =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%d:%@:1:2:3:4:%d:", currentPageNO, kL1, kL2, tileLetterTXT[tileNO],tileSetIDX[tileNO] ];
    
    // only current page number, letterNM and set number is active. content page number -- not book page number.

    [workingPageString[currentPageNO]  appendFormat:@"A-B-C Picture Book - %@  - page %d:",tileSetNM[tileNO], currentPageNO];  // abc addition
    
    [self appendCurrentTileInfo:workingPageString[currentPageNO] tile:0 lang1:kL1 lang2:kL2];  // YES GET that first tile!! only done once here.
    

//    NSLog(@"Finished first tile... string now: %@", workingPageString[currentPageNO]); // debug
...
//   add first tile to currentCOntentPageString
     ...
     //    L1 code (kCN, kEN, etc or 0 thru 7 for 8 languages)
     //    L2 code (kCN, kEN, etc or 9 thru 8 for 8 languages)  (if dupe of L1, L2 is not processed)
     //    string for char: Aa, Bb, etc.  (used to be number of tiles on this page, 4, not used. 
    // set number (1 to 16)
     //    Name of the current set (as page title)
     //    1:2:3:4:  these are the (up to) 4 tile IDs for this page; zero means no tile
     //
...
//            NSLog(@"i is now %d, and currentPageNO is %d, and tileNO is %d.", i, currentPageNO, tileNO);
            
            if (!(currentSetNO == tileSetIDX[i])) { // we have a new set.
...
//                NSLog(@"Just added NSString to pageString >>>>>>>>>>>>>>> %d, %@<", currentPageNO, workingPageString[currentPageNO]);
                currentPageNO++;
//                currentSetNO = tileSetIDX[i];
                currentSetNO++;
//                tileNO = tileID_db[i];
                tileNO++;
...
//                workingPageString[currentPageNO] =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%d:%@:1:2:3:4:%d:", currentPageNO, kL1, kL2, tileLetterTXT[currentPageNO], 1];
                
                workingPageString[currentPageNO] =  [[NSMutableString alloc] initWithFormat: @"%d:%d:%d:%@:1:2:3:4:%d:", currentPageNO, kL1, kL2, tileLetterTXT[tileNO - 1],tileSetIDX[tileNO - 1] ];  // added minuse 1  debug 
                
//                NSLog(@"===>Starting new page string as %@<", workingPageString[currentPageNO]);
//                NSLog(@"and current i value is %d", i);
                
                
                // only current page number and set number is active. content page number -- not book page number.
                
                [workingPageString[currentPageNO]  appendFormat:@"A-B-C Picture Book - %@  - page %d:",tileSetNM[tileNO - 1], currentPageNO];  // abc addition
...
//                NSLog(@"AAA currentContentPageSTring is now:%@", workingPageString[currentPageNO]); // debug
                ...
                // special case: what if last tile starts new set / page -- never got to final page rountine..
                
                if (i == 69) {
//                    NSLog(@"Ugly special case for a-b-c letter book. last tile is one tile per page z page.");
...
//                NSLog(@"process tile  old vars:  i= %d, prior tileNO>%d", i, tileNO);  // trackhal -> used for msot tiles but first / last
...
//                NSLog(@"process final  tile  old vars:  i= %d, tileNO>%d", i, tileNO);  // trackhal  not used.. 
...
- (void)viewDidLoad
{
    ...
    //  note:  phone language will be ignored. 2nd lagnuage set to en. but phone language used to populate language names on settings page if present.
...
//        databaseName = kDatabaseNameL24EN;
    ...
    k_LPhone = k_L1;   // use this for lables on settings page.
...
// 1. get L1 value from settings; set to DEFAULT  if no value (but DEFAULT is DEFAULT)
    ...
    // new for ABC. controls lables on about page...
    
    if (![defaults objectForKey:kPhoneLang])
        [defaults setObject:phoneLanguage forKey:kPhoneLang];
    
    LPhoneString_Setting = [defaults objectForKey:kPhoneLang];
...
// kL1RomanizationChoice    not used in A-B-C
/*
...
// again note L2 romanization default is no since presumed to be first language; kL1 is always english in this app.

    if (![defaults boolForKey:kL2RomanizationChoice])
//        NSLog(@"no kL2Romanization set; make NO");
...
//    NSLog(@"k_L1 value is finally set as: %@<<", k_L1);
    ...
    if ([k_LPhone isEqualToString: kLCN]) {
        kLPhone = kCN;
    } else if ([k_LPhone isEqualToString: kLDE]) {
        kLPhone = kDE;
    } else if ([k_LPhone isEqualToString: kLEN]) {
        kLPhone = kEN;
    } else if ([k_LPhone isEqualToString: kLES]) {
        kLPhone = kES;
    } else if ([k_LPhone isEqualToString: kLFR]) {
        kLPhone = kFR;
    } else if ([k_LPhone isEqualToString: kLIT]) {
        kLPhone = kIT;
    } else if ([k_LPhone isEqualToString: kLJP]) {
        kLPhone = kJP;
    } else if ([k_LPhone isEqualToString: kLKR]) {
        kLPhone = kKR;
    } else  {
        kLPhone = kEN;
    }
...
//    NSLog(@"for A-B-C letter game initial version, set language to ENGLISH!!");
    
    kL1 = kEN;  //   version 1.0 of a-b-c letter game is english only. kL2 is ignored by display aspects of program -- all items for L2 hidden...

    
    
//    NSLog(@"End of language settings in view did load.  Kl1, kL2 and kPhone are %d, %d, and %d.", kL1, kL2, kLPhone); 
    
//  kL2 is not used unless set to non english value and non defalt value...
...
//    [defaults synchronize]; // probably supurflous....
    ...
    databaseName = kDatabaseNameLW;  // was set too early
...
//    NSLog(@"End of view did load.  Kl1, kL2 and kPhone are %d, %d, and %d.", kL1, kL2, kLPhone);
